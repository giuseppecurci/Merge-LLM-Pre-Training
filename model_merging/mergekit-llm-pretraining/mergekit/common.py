...
def apply_noise(models_weight: torch.Tensor, noise_scale: float):
    """
    models_weight: tensor of shape [K, *param_shape]
    Adds Gaussian noise scaled per model.
    """
    if models_weight.dim() < 2:
        raise ValueError(f"Expected at least 2 dims [K, ...], got {models_weight.shape}")

    K = models_weight.size(0)
    param_shape = models_weight.shape[1:]
    numel = models_weight[0].numel()

    flat = models_weight.view(K, -1)
    norms = flat.norm(dim=1, keepdim=True)  # [K x 1]

    scale = noise_scale * norms / (numel**0.5)    # [K x 1]
    scale = scale.view(K, *([1] * len(param_shape)))

    noise = torch.randn_like(models_weight) * scale
    return models_weight + noise